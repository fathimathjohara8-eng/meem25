...
import { Plus, Trash2, CheckCircle, Loader2, Image, Video, AlertTriangle, Save, Settings } from 'lucide-react';
import { supabase } from '../../lib/supabase';
import { HomepageBackground, SiteSetting } from '../../types';
import { useSiteSettings } from '../../context/SiteSettingsContext';

const SiteSettingsManager: React.FC = () => {
    const { settings: initialSettings, loading: loadingSettings, refreshSettings } = useSiteSettings();
    const [settings, setSettings] = useState<Record<string, string>>({});
    const [isSaving, setIsSaving] = useState(false);

    useEffect(() => {
        if (!loadingSettings) {
            const editableSettings: Record<string, string> = {};
            if (initialSettings.logo_url) editableSettings.logo_url = initialSettings.logo_url;
            if (initialSettings.site_title) editableSettings.site_title = initialSettings.site_title;
    ...
    if (loadingSettings) {
        return <div className="flex justify-center items-center h-48"><Loader2 className="w-8 h-8 animate-spin text-brand-mid-blue" /></div>;
    }

    return (
        <div className="bg-ui-surface rounded-lg shadow-md p-6">
            <div className="flex items-center gap-3 mb-6">
                <Settings className="w-6 h-6 text-brand-mid-blue" />
                <h3 className="text-xl font-bold text-ui-text-primary">Site Settings</h3>
            </div>
            <div className="space-y-4">
                <div>
                    <label className="block text-sm font-medium text-ui-text-secondary mb-1">Site Title</label>
                    <input 
                        type="text" 
                        value={settings.site_title || ''}
                        onChange={e => handleSettingChange('site_title', e.target.value)}
                        className="w-full px-3 py-2 border border-black/10 rounded-lg bg-ui-background"
                    />
                </div>
                <div>
                    <label className="block text-sm font-medium text-ui-text-secondary mb-1">Logo URL</label>
                    <input 
                        type="url" 
            ...
            <div className="mt-6 flex justify-end">
                <button
                    onClick={handleSave}
                    disabled={isSaving}
                    className="bg-brand-mid-blue text-ui-text-light px-4 py-2 rounded-lg hover:bg-brand-dark-blue transition-colors flex items-center gap-2 disabled:opacity-50"
                >
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-ui-text-primary">Manage Homepage & Settings</h2>
          <p className="text-ui-text-secondary">Manage backgrounds and global site settings.</p>
        </div>
      </div>
      ...
      <div className="mt-8">
        <div className="flex items-center justify-between">
            <div>
            <h2 className="text-xl font-bold text-ui-text-primary">Homepage Background</h2>
