...
const AdminResults: React.FC = () => {
    ...
    try {
      ...
      const resultsToInsert = winnersData.winners
        ...
        .map(winner => ({
          ...
          result_number: parseInt(winnersData.result_number, 10) || 0,
        }));
  ...
  const handleEdit = (competition: { event: string; category: string; year: string; winners: Result[] }) => {
    ...
    const firstWinner = winnersFromDb[0];
    ...
    setWinnersData({
      ...
      result_number: firstWinner ? String(firstWinner.result_number) : '',
  ...
  const handleDownload = () => {
    ...
    const headers = ['ID', 'Event', 'Category', 'Year', 'Result Number', 'Participant', 'School', 'Position', 'Created At'];
    const csvContent = [
      headers.join(','),
            ...
            <div key={`${group.event}-${group.category}-${group.year}`} className="bg-ui-surface rounded-lg shadow-md p-6">
              <div className="flex items-center justify-between mb-4 border-b pb-4">
                ...
                <div className="flex items-center space-x-2">
                    <span className="font-bold text-brand-mid-blue text-lg">#{group.result_number}</span>
                  <button
          ...
          <div className="bg-ui-surface rounded-lg p-8 w-full max-w-4xl max-h-[95vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Result Number <span className="text-rose-500">*</span>
                  </label>
                  <input
                    type="number"
                    value={winnersData.result_number}
                    onChange={(e) => setWinnersData({...winnersData, result_number: e.target.value})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-mid-blue focus:border-transparent"
                    placeholder="e.g., 101"
                    required
                  />
                </div>
